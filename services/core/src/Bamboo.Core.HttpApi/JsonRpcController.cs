    ...
    public class JsonRpcController : AbpController
    {
        ...
        private async Task<object> ProcessRequestAsync(JsonRpcRequest request)
        {
            ...
            switch (method)
            {
                case "search":
                    {
                        var domain = kwargs.ContainsKey("domain") ? JsonSerializer.Serialize(kwargs["domain"], jsonOptions) : args.Count > 0 ? JsonSerializer.Serialize(args[0], jsonOptions) : null;
                ...
                case "name_search":
                    {
                        ...
                        var domain = kwargs.ContainsKey("domain") ? JsonSerializer.Serialize(kwargs["domain"], jsonOptions) : args.Count > 1 ? JsonSerializer.Serialize(args[1], jsonOptions) : null;
