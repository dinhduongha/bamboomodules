    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<IrActUrl>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("ir_act_url_pkey");
                        ...
                        entity.HasOne(d => d.BindingModel).WithMany(p => p.IrActUrl)
                            .HasForeignKey(d => d.BindingModelId)
                            .OnDelete(DeleteBehavior.Cascade)
                            .HasConstraintName("ir_act_url_binding_model_id_fkey");

                        // entity.HasOne(d => d.CreateU).WithMany(p => p.IrActUrlCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("ir_act_url_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("ir_act_url_create_uid_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.IrActUrlWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("ir_act_url_write_uid_fkey");
