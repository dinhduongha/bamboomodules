    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AuthTotpRateLimitLog>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("auth_totp_rate_limit_log_pkey");
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.AuthTotpRateLimitLogCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("auth_totp_rate_limit_log_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("auth_totp_rate_limit_log_create_uid_fkey");

                        // entity.HasOne(d => d.User).WithMany(p => p.AuthTotpRateLimitLogUser) .HasForeignKey(d => d.UserId) .OnDelete(DeleteBehavior.Cascade) .HasConstraintName("auth_totp_rate_limit_log_user_id_fkey");
                        entity.HasOne(d => d.User).WithMany()
                            .HasForeignKey(d => d.UserId)
                            .OnDelete(DeleteBehavior.Cascade)
                            .HasConstraintName("auth_totp_rate_limit_log_user_id_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.AuthTotpRateLimitLogWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("auth_totp_rate_limit_log_write_uid_fkey");
