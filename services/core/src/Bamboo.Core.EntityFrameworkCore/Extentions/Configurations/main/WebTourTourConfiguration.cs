    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<WebTourTour>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("web_tour_tour_pkey");
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.WebTourTourCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("web_tour_tour_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("web_tour_tour_create_uid_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.WebTourTourWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("web_tour_tour_write_uid_fkey");
                        entity.HasOne(d => d.WriteU).WithMany()
                            .HasForeignKey(d => d.LastModifierId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("web_tour_tour_write_uid_fkey");

                        // entity.HasMany(d => d.ResUsers).WithMany(p => p.WebTourTour)
