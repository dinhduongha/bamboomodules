    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<BarcodeRule>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("barcode_rule_pkey");
                        ...
                        entity.HasIndex(e => e.BarcodeNomenclatureId, "barcode_rule__barcode_nomenclature_id_index").HasFilter("(barcode_nomenclature_id IS NOT NULL)");
                        ...
                        // entity.HasOne(d => d.AssociatedUom).WithMany(p => p.BarcodeRule) .HasForeignKey(d => d.AssociatedUomId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("barcode_rule_associated_uom_id_fkey");
                        entity.HasOne(d => d.AssociatedUom).WithMany()
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.BarcodeRuleCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("barcode_rule_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("barcode_rule_create_uid_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.BarcodeRuleWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("barcode_rule_write_uid_fkey");
