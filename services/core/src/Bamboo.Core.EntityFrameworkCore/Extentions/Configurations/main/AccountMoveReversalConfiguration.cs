    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountMoveReversal>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("account_move_reversal_pkey");
                        ...
                        // entity.HasOne(d => d.Company).WithMany(p => p.AccountMoveReversal) .HasForeignKey(d => d.TenantId) .OnDelete(DeleteBehavior.Cascade) .HasConstraintName("account_move_reversal_company_id_fkey");
                        entity.HasOne(d => d.Company).WithMany()
                            .HasForeignKey(d => d.TenantId)
                            .OnDelete(DeleteBehavior.Cascade)
                            .HasConstraintName("account_move_reversal_company_id_fkey");

                        // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountMoveReversalCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_move_reversal_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_move_reversal_create_uid_fkey");

                        // entity.HasOne(d => d.Journal).WithMany(p => p.AccountMoveReversal) .HasForeignKey(d => d.JournalId) .OnDelete(DeleteBehavior.Cascade) .HasConstraintName("account_move_reversal_journal_id_fkey");
