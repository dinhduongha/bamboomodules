...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ProductComboItem>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.ProductComboItem)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("product_combo_item_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.ProductComboItemCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("product_combo_item_create_uid_fkey");

            // entity.HasOne(d => d.Product).WithMany(p => p.ProductComboItem)
            entity.HasOne(d => d.Product).WithMany()
                .HasForeignKey(d => d.ProductId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("product_combo_item_product_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ProductComboItemWriteU)
