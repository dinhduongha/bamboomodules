...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountAutopostBillsWizard>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountAutopostBillsWizardCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_autopost_bills_wizard_create_uid_fkey");

            // entity.HasOne(d => d.Partner).WithMany(p => p.AccountAutopostBillsWizard)
            entity.HasOne(d => d.Partner).WithMany()
                .HasForeignKey(d => d.PartnerId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_autopost_bills_wizard_partner_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountAutopostBillsWizardWriteU)
