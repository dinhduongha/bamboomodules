...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ProductCombo>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.ProductCombo)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("product_combo_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.ProductComboCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("product_combo_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ProductComboWriteU)
