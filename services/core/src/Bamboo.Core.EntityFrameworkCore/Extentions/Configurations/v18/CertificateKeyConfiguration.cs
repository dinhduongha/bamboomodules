...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<CertificateKey>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.CertificateKey)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("certificate_key_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.CertificateKeyCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("certificate_key_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.CertificateKeyWriteU)
