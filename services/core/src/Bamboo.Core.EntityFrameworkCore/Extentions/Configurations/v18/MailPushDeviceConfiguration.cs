...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<MailPushDevice>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.MailPushDeviceCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("mail_push_device_create_uid_fkey");

            // entity.HasOne(d => d.Partner).WithMany(p => p.MailPushDevice)
            entity.HasOne(d => d.Partner).WithMany()
                .HasForeignKey(d => d.PartnerId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("mail_push_device_partner_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.MailPushDeviceWriteU)
