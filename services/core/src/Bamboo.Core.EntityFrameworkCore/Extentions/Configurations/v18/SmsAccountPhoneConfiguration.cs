...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<SmsAccountPhone>(entity =>
            ...
            // entity.HasOne(d => d.Account).WithMany(p => p.SmsAccountPhone)
            entity.HasOne(d => d.Account).WithMany()
                .HasForeignKey(d => d.AccountId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("sms_account_phone_account_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.SmsAccountPhoneCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("sms_account_phone_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.SmsAccountPhoneWriteU)
