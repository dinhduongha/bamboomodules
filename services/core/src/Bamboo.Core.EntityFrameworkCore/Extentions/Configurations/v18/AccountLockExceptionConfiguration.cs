...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountLockException>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountLockException)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("account_lock_exception_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountLockExceptionCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_lock_exception_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.AccountLockExceptionUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_lock_exception_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountLockExceptionWriteU)
