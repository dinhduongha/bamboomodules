...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<SmsAccountSender>(entity =>
            ...
            // entity.HasOne(d => d.Account).WithMany(p => p.SmsAccountSender)
            entity.HasOne(d => d.Account).WithMany()
                .HasForeignKey(d => d.AccountId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("sms_account_sender_account_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.SmsAccountSenderCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("sms_account_sender_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.SmsAccountSenderWriteU)
