...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<MailActivityTodoCreate>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.MailActivityTodoCreateCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("mail_activity_todo_create_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.MailActivityTodoCreateUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("mail_activity_todo_create_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.MailActivityTodoCreateWriteU)
