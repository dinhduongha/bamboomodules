...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<PeppolRegistration>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.PeppolRegistration)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("peppol_registration_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.PeppolRegistrationCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("peppol_registration_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.PeppolRegistrationWriteU)
