...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<PosPrinter>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.PosPrinter)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("pos_printer_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.PosPrinterCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("pos_printer_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.PosPrinterWriteU)
