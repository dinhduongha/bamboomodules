    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountReconcileModelTemplate>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("account_reconcile_model_template_pkey");
                        ...
                        entity.HasOne(d => d.ChartTemplate).WithMany(p => p.AccountReconcileModelTemplate)
                            .HasForeignKey(d => d.ChartTemplateId)
                            .OnDelete(DeleteBehavior.Restrict)
                            .HasConstraintName("account_reconcile_model_template_chart_template_id_fkey");

                        // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountReconcileModelTemplateCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_reconcile_model_template_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_reconcile_model_template_create_uid_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountReconcileModelTemplateWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_reconcile_model_template_write_uid_fkey");
                        entity.HasOne(d => d.WriteU).WithMany()
                            .HasForeignKey(d => d.LastModifierId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_reconcile_model_template_write_uid_fkey");

                        // entity.HasMany(d => d.AccountJournal).WithMany(p => p.AccountReconcileModelTemplate)
                        entity.HasMany(d => d.AccountJournal).WithMany(p => p.AccountReconcileModelTemplate)
                            .UsingEntity<Dictionary<string, object>>(
                                ...
                                j =>
                                {
                                    ...
                                    j.IndexerProperty<Guid>("AccountReconcileModelTemplateId").HasColumnName("account_reconcile_model_template_id");
                                    j.IndexerProperty<Guid>("AccountJournalId").HasColumnName("account_journal_id");
                                });

                        // entity.HasMany(d => d.ResPartner).WithMany(p => p.AccountReconcileModelTemplate)
                        ...
                        // entity.HasMany(d => d.ResPartnerCategory).WithMany(p => p.AccountReconcileModelTemplate)
                        entity.HasMany(d => d.ResPartnerCategory).WithMany(p => p.AccountReconcileModelTemplate)
                            .UsingEntity<Dictionary<string, object>>(
