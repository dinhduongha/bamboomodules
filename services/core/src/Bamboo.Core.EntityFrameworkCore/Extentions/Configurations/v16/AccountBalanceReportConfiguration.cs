...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountBalanceReport>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountBalanceReport)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("account_balance_report_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountBalanceReportCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_balance_report_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountBalanceReportWriteU)
