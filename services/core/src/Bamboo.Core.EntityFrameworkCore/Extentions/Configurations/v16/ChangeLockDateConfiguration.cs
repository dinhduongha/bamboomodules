...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ChangeLockDate>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.ChangeLockDate)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("change_lock_date_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.ChangeLockDateCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("change_lock_date_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ChangeLockDateWriteU)
