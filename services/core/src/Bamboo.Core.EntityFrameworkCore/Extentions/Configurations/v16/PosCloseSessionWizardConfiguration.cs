...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<PosCloseSessionWizard>(entity =>
            ...
            // entity.HasOne(d => d.Account).WithMany(p => p.PosCloseSessionWizard)
            entity.HasOne(d => d.Account).WithMany()
                .HasForeignKey(d => d.AccountId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("pos_close_session_wizard_account_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.PosCloseSessionWizardCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("pos_close_session_wizard_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.PosCloseSessionWizardWriteU)
