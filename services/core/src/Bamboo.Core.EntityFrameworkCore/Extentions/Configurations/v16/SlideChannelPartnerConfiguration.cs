    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<SlideChannelPartner>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("slide_channel_partner_pkey");
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.SlideChannelPartnerCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("slide_channel_partner_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("slide_channel_partner_create_uid_fkey");

                        // entity.HasOne(d => d.Partner).WithMany(p => p.SlideChannelPartner) .HasForeignKey(d => d.PartnerId) .OnDelete(DeleteBehavior.Cascade) .HasConstraintName("slide_channel_partner_partner_id_fkey");
                        entity.HasOne(d => d.Partner).WithMany()
                            .HasForeignKey(d => d.PartnerId)
                            .OnDelete(DeleteBehavior.Cascade)
                            .HasConstraintName("slide_channel_partner_partner_id_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.SlideChannelPartnerWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("slide_channel_partner_write_uid_fkey");
