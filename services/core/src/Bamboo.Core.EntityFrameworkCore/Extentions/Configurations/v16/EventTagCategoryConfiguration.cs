...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<EventTagCategory>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.EventTagCategoryCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("event_tag_category_create_uid_fkey");

            // entity.HasOne(d => d.Website).WithMany(p => p.EventTagCategory)
            entity.HasOne(d => d.Website).WithMany()
                .HasForeignKey(d => d.WebsiteId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("event_tag_category_website_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.EventTagCategoryWriteU)
