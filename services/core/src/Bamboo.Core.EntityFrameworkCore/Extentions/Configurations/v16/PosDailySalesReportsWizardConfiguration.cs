    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<PosDailySalesReportsWizard>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("pos_daily_sales_reports_wizard_pkey");
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.PosDailySalesReportsWizardCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("pos_daily_sales_reports_wizard_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("pos_daily_sales_reports_wizard_create_uid_fkey");

                        entity.HasOne(d => d.PosSession).WithMany(p => p.PosDailySalesReportsWizard)
                            .HasForeignKey(d => d.PosSessionId)
                            .OnDelete(DeleteBehavior.Cascade)
                            .HasConstraintName("pos_daily_sales_reports_wizard_pos_session_id_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.PosDailySalesReportsWizardWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("pos_daily_sales_reports_wizard_write_uid_fkey");
