...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<MembershipInvoice>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.MembershipInvoiceCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("membership_invoice_create_uid_fkey");

            // entity.HasOne(d => d.Product).WithMany(p => p.MembershipInvoice)
            entity.HasOne(d => d.Product).WithMany()
                .HasForeignKey(d => d.ProductId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("membership_invoice_product_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.MembershipInvoiceWriteU)
