...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ProductPackaging>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.ProductPackaging)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("product_packaging_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.ProductPackagingCreateU)
            ...
            // entity.HasOne(d => d.Product).WithMany(p => p.ProductPackaging)
            entity.HasOne(d => d.Product).WithMany()
                .HasForeignKey(d => d.ProductId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("product_packaging_product_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ProductPackagingWriteU)
