...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountTaxReportWizard>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountTaxReportWizard)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("account_tax_report_wizard_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountTaxReportWizardCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_tax_report_wizard_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountTaxReportWizardWriteU)
