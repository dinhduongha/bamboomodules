...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AssetDepreciationConfirmationWizard>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.AssetDepreciationConfirmationWizardCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("asset_depreciation_confirmation_wizard_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AssetDepreciationConfirmationWizardWriteU)
