...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<CrmLeadForwardToPartner>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.CrmLeadForwardToPartnerCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("crm_lead_forward_to_partner_create_uid_fkey");

            // entity.HasOne(d => d.Partner).WithMany(p => p.CrmLeadForwardToPartner)
            entity.HasOne(d => d.Partner).WithMany()
                .HasForeignKey(d => d.PartnerId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("crm_lead_forward_to_partner_partner_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.CrmLeadForwardToPartnerWriteU)
