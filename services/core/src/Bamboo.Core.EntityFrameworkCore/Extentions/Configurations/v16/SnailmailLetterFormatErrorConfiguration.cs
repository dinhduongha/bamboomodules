    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<SnailmailLetterFormatError>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("snailmail_letter_format_error_pkey");
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.SnailmailLetterFormatErrorCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("snailmail_letter_format_error_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("snailmail_letter_format_error_create_uid_fkey");

                        entity.HasOne(d => d.Message).WithMany(p => p.SnailmailLetterFormatError)
                            .HasForeignKey(d => d.MessageId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("snailmail_letter_format_error_message_id_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.SnailmailLetterFormatErrorWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("snailmail_letter_format_error_write_uid_fkey");
