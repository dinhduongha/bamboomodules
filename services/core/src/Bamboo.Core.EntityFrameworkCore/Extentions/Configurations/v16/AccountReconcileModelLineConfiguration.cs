    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountReconcileModelLine>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("account_reconcile_model_line_pkey");
                        ...
                        // entity.HasOne(d => d.Account).WithMany(p => p.AccountReconcileModelLine) .HasForeignKey(d => d.AccountId) .OnDelete(DeleteBehavior.Cascade) .HasConstraintName("account_reconcile_model_line_account_id_fkey");
                        entity.HasOne(d => d.Account).WithMany()
                            .HasForeignKey(d => d.AccountId)
                            .OnDelete(DeleteBehavior.Cascade)
                            .HasConstraintName("account_reconcile_model_line_account_id_fkey");

                        // entity.HasOne(d => d.Company).WithMany(p => p.AccountReconcileModelLine) .HasForeignKey(d => d.TenantId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_reconcile_model_line_company_id_fkey");
                        entity.HasOne(d => d.Company).WithMany()
                            .HasForeignKey(d => d.TenantId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_reconcile_model_line_company_id_fkey");

                        // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountReconcileModelLineCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_reconcile_model_line_create_uid_fkey");
