...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<RecurringPayment>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.RecurringPayment)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("recurring_payment_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.RecurringPaymentCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("recurring_payment_create_uid_fkey");

            // entity.HasOne(d => d.Partner).WithMany(p => p.RecurringPayment)
            ...
            // entity.HasOne(d => d.WriteU).WithMany(p => p.RecurringPaymentWriteU)
