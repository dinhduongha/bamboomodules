...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountBudgetPost>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountBudgetPost)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("account_budget_post_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountBudgetPostCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_budget_post_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountBudgetPostWriteU)
