...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ResCity>(entity =>
            ...
            // entity.HasOne(d => d.Country).WithMany(p => p.ResCity)
            entity.HasOne(d => d.Country).WithMany()
                .HasForeignKey(d => d.CountryId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("res_city_country_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.ResCityCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("res_city_create_uid_fkey");

            entity.HasOne(d => d.State).WithMany(p => p.ResCity)
                .HasForeignKey(d => d.StateId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("res_city_state_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ResCityWriteU)
