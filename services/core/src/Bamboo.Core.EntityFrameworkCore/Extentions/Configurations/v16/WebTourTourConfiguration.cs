...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<WebTourTour>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.WebTourTourCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("web_tour_tour_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.WebTourTour)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("web_tour_tour_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.WebTourTourWriteU)
