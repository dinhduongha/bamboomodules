...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountReconcileModel>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountReconcileModel)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("account_reconcile_model_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountReconcileModelCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_reconcile_model_create_uid_fkey");

            // entity.HasOne(d => d.MessageMainAttachment).WithMany(p => p.AccountReconcileModel)
            ...
            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountReconcileModelWriteU)
