    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ProjectTaskUserRel>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("project_task_user_rel_pkey");
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.ProjectTaskUserRelCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("project_task_user_rel_create_uid_fkey");
                        ...
                        // entity.HasOne(d => d.User).WithMany(p => p.ProjectTaskUserRelUser) .HasForeignKey(d => d.UserId) .OnDelete(DeleteBehavior.Cascade) .HasConstraintName("project_task_user_rel_user_id_fkey");
                        entity.HasOne(d => d.User).WithMany()
                            .HasForeignKey(d => d.UserId)
                            .OnDelete(DeleteBehavior.Cascade)
                            .HasConstraintName("project_task_user_rel_user_id_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.ProjectTaskUserRelWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("project_task_user_rel_write_uid_fkey");
