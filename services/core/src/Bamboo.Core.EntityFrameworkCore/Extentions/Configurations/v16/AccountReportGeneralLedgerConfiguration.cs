...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountReportGeneralLedger>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountReportGeneralLedger)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("account_report_general_ledger_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountReportGeneralLedgerCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_report_general_ledger_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountReportGeneralLedgerWriteU)
