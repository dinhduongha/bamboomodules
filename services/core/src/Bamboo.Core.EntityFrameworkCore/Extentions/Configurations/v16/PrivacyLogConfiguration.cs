...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<PrivacyLog>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.PrivacyLogCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("privacy_log_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.PrivacyLogUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("privacy_log_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.PrivacyLogWriteU)
