...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AuthTotpWizard>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.AuthTotpWizardCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("auth_totp_wizard_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.AuthTotpWizardUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("auth_totp_wizard_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AuthTotpWizardWriteU)
