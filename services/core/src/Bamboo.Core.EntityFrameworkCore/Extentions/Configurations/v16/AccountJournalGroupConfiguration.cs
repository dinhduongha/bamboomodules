    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountJournalGroup>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("account_journal_group_pkey");
                        ...
                        // entity.HasOne(d => d.Company).WithMany(p => p.AccountJournalGroup) .HasForeignKey(d => d.TenantId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_journal_group_company_id_fkey");
                        entity.HasOne(d => d.Company).WithMany()
                            .HasForeignKey(d => d.TenantId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_journal_group_company_id_fkey");

                        // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountJournalGroupCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_journal_group_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_journal_group_create_uid_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountJournalGroupWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_journal_group_write_uid_fkey");
