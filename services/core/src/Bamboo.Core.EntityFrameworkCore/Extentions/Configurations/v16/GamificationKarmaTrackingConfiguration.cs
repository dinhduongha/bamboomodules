...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<GamificationKarmaTracking>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.GamificationKarmaTrackingCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("gamification_karma_tracking_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.GamificationKarmaTrackingUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("gamification_karma_tracking_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.GamificationKarmaTrackingWriteU)
