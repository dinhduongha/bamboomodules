...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<IrSequence>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.IrSequence)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("ir_sequence_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.IrSequenceCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("ir_sequence_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.IrSequenceWriteU)
