...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<SlideSlide>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.SlideSlideCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("slide_slide_create_uid_fkey");

            // entity.HasOne(d => d.MessageMainAttachment).WithMany(p => p.SlideSlide)
            ...
            // entity.HasOne(d => d.User).WithMany(p => p.SlideSlideUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("slide_slide_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.SlideSlideWriteU)
