...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountingReport>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountingReport)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("accounting_report_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountingReportCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("accounting_report_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountingReportWriteU)
