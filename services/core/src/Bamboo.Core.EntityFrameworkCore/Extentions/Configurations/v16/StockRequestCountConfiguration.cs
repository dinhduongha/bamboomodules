...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<StockRequestCount>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.StockRequestCountCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("stock_request_count_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.StockRequestCountUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("stock_request_count_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.StockRequestCountWriteU)
