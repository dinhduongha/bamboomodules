...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<SalePaymentProviderOnboardingWizard>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.SalePaymentProviderOnboardingWizardCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("sale_payment_provider_onboarding_wizard_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.SalePaymentProviderOnboardingWizardWriteU)
