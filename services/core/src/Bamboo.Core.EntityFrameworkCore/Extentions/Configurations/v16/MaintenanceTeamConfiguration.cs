...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<MaintenanceTeam>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.MaintenanceTeam)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("maintenance_team_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.MaintenanceTeamCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("maintenance_team_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.MaintenanceTeamWriteU)
