...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountCommonReport>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountCommonReport)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("account_common_report_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountCommonReportCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_common_report_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountCommonReportWriteU)
