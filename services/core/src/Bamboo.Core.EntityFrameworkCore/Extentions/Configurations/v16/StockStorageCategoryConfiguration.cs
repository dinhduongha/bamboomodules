...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<StockStorageCategory>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.StockStorageCategory)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("stock_storage_category_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.StockStorageCategoryCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("stock_storage_category_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.StockStorageCategoryWriteU)
