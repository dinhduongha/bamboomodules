...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ResUsersDeletion>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.ResUsersDeletionCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("res_users_deletion_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.ResUsersDeletionUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("res_users_deletion_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ResUsersDeletionWriteU)
