...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<SurveySurvey>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.SurveySurveyCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("survey_survey_create_uid_fkey");

            // entity.HasOne(d => d.MessageMainAttachment).WithMany(p => p.SurveySurvey)
            ...
            // entity.HasOne(d => d.User).WithMany(p => p.SurveySurveyUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("survey_survey_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.SurveySurveyWriteU)
