...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<LunchLocation>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.LunchLocation)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("lunch_location_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.LunchLocationCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("lunch_location_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.LunchLocationWriteU)
