...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ResourceCalendar>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.ResourceCalendarNavigation)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("resource_calendar_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.ResourceCalendarCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("resource_calendar_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ResourceCalendarWriteU)
