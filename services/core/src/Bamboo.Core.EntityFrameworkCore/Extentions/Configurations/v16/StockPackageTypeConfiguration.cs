...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<StockPackageType>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.StockPackageType)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("stock_package_type_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.StockPackageTypeCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("stock_package_type_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.StockPackageTypeWriteU)
