...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<PosPaymentMethod>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.PosPaymentMethod)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("pos_payment_method_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.PosPaymentMethodCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("pos_payment_method_create_uid_fkey");

            // entity.HasOne(d => d.Journal).WithMany(p => p.PosPaymentMethod)
            ...
            // entity.HasOne(d => d.WriteU).WithMany(p => p.PosPaymentMethodWriteU)
