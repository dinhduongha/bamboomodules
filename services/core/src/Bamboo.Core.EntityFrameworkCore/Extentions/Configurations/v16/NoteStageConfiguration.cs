...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<NoteStage>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.NoteStageCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("note_stage_create_uid_fkey");

            // entity.HasOne(d => d.User).WithMany(p => p.NoteStageUser)
            entity.HasOne(d => d.User).WithMany()
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("note_stage_user_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.NoteStageWriteU)
