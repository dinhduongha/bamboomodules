...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<ResBank>(entity =>
            ...
            // entity.HasOne(d => d.CreateU).WithMany(p => p.ResBankCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("res_bank_create_uid_fkey");

            entity.HasOne(d => d.StateNavigation).WithMany(p => p.ResBank)
                .HasForeignKey(d => d.State)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("res_bank_state_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.ResBankWriteU)
