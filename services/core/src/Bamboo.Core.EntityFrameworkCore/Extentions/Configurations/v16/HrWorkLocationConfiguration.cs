...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<HrWorkLocation>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.HrWorkLocation)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("hr_work_location_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.HrWorkLocationCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("hr_work_location_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.HrWorkLocationWriteU)
