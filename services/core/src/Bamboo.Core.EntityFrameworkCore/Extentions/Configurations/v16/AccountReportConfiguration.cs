    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountReport>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("account_report_pkey");
                        ...
                        // entity.HasOne(d => d.Country).WithMany(p => p.AccountReport) .HasForeignKey(d => d.CountryId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_report_country_id_fkey");
                        entity.HasOne(d => d.Country).WithMany()
                            .HasForeignKey(d => d.CountryId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_report_country_id_fkey");

                        // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountReportCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_report_create_uid_fkey");
                        ...
                        // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountReportWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("account_report_write_uid_fkey");
                        entity.HasOne(d => d.WriteU).WithMany()
                            .HasForeignKey(d => d.LastModifierId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("account_report_write_uid_fkey");

                        // entity.HasMany(d => d.MainReport).WithMany(p => p.SubReport)
                        entity.HasMany(d => d.MainReport).WithMany(p => p.SubReport)
                        ...
                        // entity.HasMany(d => d.SubReport).WithMany(p => p.MainReport)
                        entity.HasMany(d => d.SubReport).WithMany(p => p.MainReport)
                            .UsingEntity<Dictionary<string, object>>(
                                "AccountReportSectionRel",
                                r => r.HasOne<AccountReport>().WithMany()
