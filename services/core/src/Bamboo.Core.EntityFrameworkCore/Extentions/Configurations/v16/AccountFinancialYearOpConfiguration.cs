...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountFinancialYearOp>(entity =>
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountFinancialYearOp)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Cascade)
                .HasConstraintName("account_financial_year_op_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountFinancialYearOpCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_financial_year_op_create_uid_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountFinancialYearOpWriteU)
