...
namespace Bamboo.Core.EntityFrameworkCore
{
    ...
    {
        public static void ConfigureCrossoveredBudgetLines(this ModelBuilder modelBuilder)
        {
        modelBuilder.Entity<CrossoveredBudgetLines>(entity =>
            {
            entity.HasKey(e => e.Id).HasName("crossovered_budget_lines_pkey");
                        ...
                        entity.HasIndex(e => e.CrossoveredBudgetId, "crossovered_budget_lines__crossovered_budget_id_index");
                        ...
                        // entity.HasOne(d => d.Company).WithMany(p => p.CrossoveredBudgetLines) .HasForeignKey(d => d.TenantId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("crossovered_budget_lines_company_id_fkey");
                        entity.HasOne(d => d.Company).WithMany()
                            .HasForeignKey(d => d.TenantId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("crossovered_budget_lines_company_id_fkey");

                        // entity.HasOne(d => d.CreateU).WithMany(p => p.CrossoveredBudgetLinesCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("crossovered_budget_lines_create_uid_fkey");
                        ...
                        // entity.HasOne(d => d.WriteU).WithMany(p => p.CrossoveredBudgetLinesWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("crossovered_budget_lines_write_uid_fkey");
