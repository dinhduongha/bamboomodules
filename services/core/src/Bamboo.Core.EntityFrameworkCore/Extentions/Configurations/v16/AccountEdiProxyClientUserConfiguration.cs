...
namespace Bamboo.Core.EntityFrameworkCore
{
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<AccountEdiProxyClientUser>(entity =>
            ...
            entity.HasIndex(e => new { e.EdiIdentification, e.EdiFormatId }, "account_edi_proxy_client_user_unique_edi_identification_per_for").IsUnique();
            ...
            // entity.HasOne(d => d.Company).WithMany(p => p.AccountEdiProxyClientUser)
            entity.HasOne(d => d.Company).WithMany()
                .HasForeignKey(d => d.TenantId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("account_edi_proxy_client_user_company_id_fkey");

            // entity.HasOne(d => d.CreateU).WithMany(p => p.AccountEdiProxyClientUserCreateU)
            entity.HasOne(d => d.CreateU).WithMany()
                .HasForeignKey(d => d.CreatorId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("account_edi_proxy_client_user_create_uid_fkey");

            entity.HasOne(d => d.PrivateKeyCert).WithMany(p => p.AccountEdiProxyClientUser)
                .HasForeignKey(d => d.PrivateKeyId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("account_edi_proxy_client_user_private_key_id_fkey");

            entity.HasOne(d => d.EdiFormat).WithMany(p => p.AccountEdiProxyClientUser)
                .HasForeignKey(d => d.EdiFormatId)
                .OnDelete(DeleteBehavior.Restrict)
                .HasConstraintName("account_edi_proxy_client_user_edi_format_id_fkey");

            // entity.HasOne(d => d.WriteU).WithMany(p => p.AccountEdiProxyClientUserWriteU)
