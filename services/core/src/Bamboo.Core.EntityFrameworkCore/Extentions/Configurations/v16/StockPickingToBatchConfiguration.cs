    ...
    public static partial class ModelBuilderExtensions
    {
        ...
        modelBuilder.Entity<StockPickingToBatch>(entity =>
            ...
            entity.HasKey(e => e.Id).HasName("stock_picking_to_batch_pkey");
                        ...
                        // entity.HasOne(d => d.CreateU).WithMany(p => p.StockPickingToBatchCreateU) .HasForeignKey(d => d.CreatorId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("stock_picking_to_batch_create_uid_fkey");
                        entity.HasOne(d => d.CreateU).WithMany()
                            .HasForeignKey(d => d.CreatorId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("stock_picking_to_batch_create_uid_fkey");

                        // entity.HasOne(d => d.User).WithMany(p => p.StockPickingToBatchUser) .HasForeignKey(d => d.UserId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("stock_picking_to_batch_user_id_fkey");
                        entity.HasOne(d => d.User).WithMany()
                            .HasForeignKey(d => d.UserId)
                            .OnDelete(DeleteBehavior.SetNull)
                            .HasConstraintName("stock_picking_to_batch_user_id_fkey");

                        // entity.HasOne(d => d.WriteU).WithMany(p => p.StockPickingToBatchWriteU) .HasForeignKey(d => d.LastModifierId) .OnDelete(DeleteBehavior.SetNull) .HasConstraintName("stock_picking_to_batch_write_uid_fkey");
